...
## 11. 英文 AI 信源（权威风向 + 低粉爆款）· 近 12 小时 · 高阅读 → Discord（GoSailGlobal 搬运池文案）

- 名单：`en_sources_by_category.json`（顶级官方权威 / 低粉创作者 / 工具实操 / 独立产品复盘；**不含**媒体/通讯社）。顶级权威用于捕捉 OpenAI / DeepSeek / Cursor / Anthropic / Google DeepMind 等官方风向，低粉池用于找早期爆款。  
- 脚本：`fetch_en_top10_discord.py`  
  ...
  - 严格过滤：`lang=en`、AI/工具关键词命中、`viewCount >= 100000`、作者粉丝数 `<= 50000`；`authority_allowlist` 里的官方/权威号不受粉丝上限约束。  
